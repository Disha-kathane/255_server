...
const wabot_db1 = require('../../db/wabot1');
...
const getWaUserSettings = async (campaignId) => {
    const query = 'select `a`.`usrnm`, `a`.`usrpass`, `a`.`authts`, `a`.`authvalidity`, `a`.`authtoken`, `a`.`waurl`, `a`.`wanumber`, `a`.`hsmnamespace`,`a`.`phone_number_id`' +
        ' from `ezb_wa_msg_settings` AS a, `ezb_wa_campaign_master` as b' +
...
const getUserIdWithWabaNumber = async (wabaNumber) => {
    ...
    const query = "SELECT a.userid FROM ezb_wa_msg_settings AS a, ezb_users AS b WHERE a.wanumber = ? AND a.userid = b.userid AND b.`userstatus` = ?";
    const value = [wabaNumber, 1];
    const [result] = await wabot_db.query(query, value);
...
const getImagePath = async (messageId, mobileNo) => {
    try {
        ...
        const [result] = await wabot_db1.query(query, values);
...
const updateTPSInfo = async (display_phone_number, platform_type, level) => {
    console.log({ display_phone_number, platform_type, level })
    console.log({ display_phone_number, platform_type, level });
    const query = 'UPDATE ezb_wa_msg_settings SET platform_type = ?, throughput = ? where wanumber = ?';
    const value = [platform_type, level, "+" + display_phone_number];
    const [result] = await wabot_db.query(query, value);
    return result;
};
