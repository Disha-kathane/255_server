...
const template_getSystemAccessToken = async () => {
    const query = 'select value from ' + TBL_SYSTEM_CONFIG_MASTER + ' where paramname = ?';
    const value = ['TEMPLATE_ACCESS_TOKEN'];
    const result = await wabot_db.query(query, value);

    console.log("result1 token ----------------------------> ", result)
    return result[0];
};
...
const getWabaUserId = async (temptitle, userid, tempid) => {
    const query = 'select b.whatsapp_business_account_id AS wabaId from ezb_wa_api_keys as a, ezb_wa_msg_settings as b, ezb_wa_templates as c' +
        ' where a.userid = b.userid and b.userid = c.userid and c.temptitle = ? and c.userid = ? and c.tempid = ? and c.waba_approval_response_id is not null';
...
const getTemplateDetailsEdit = async (tempIdList, userId) => {
    const query = 'select * from ' + TBL_TEMPLATE_MASTER_EDIT + ' where tempid  = ? and userid = ?;';
    const value = [tempIdList, userId];
    const [result] = await wabot_db.query(query, value);
...
module.exports = {
    ...
    template_getSystemAccessToken,
    getTemplateDetailsEdit
};
