...
module.exports = async (req, res) => {
    try {
        ...
        let phonenumberArr = req.body.wanumber;
        // let product_type = (typeof req.body.type != undefined) ? req.body.type + '' : '';
        let data = [];

        console.log(req.body, phonenumberArr)
        ...
        console.log("inside the conversation analytics", req.body)
        ...
        let fetchMetric = (access_token, wabainfo, done) => {
            ...
            console.log(`conversation_analytics.start(${startdate}).end(${enddate}).granularity(DAILY).phone_numbers(${phonenumberArr}).dimensions(["CONVERSATION_CATEGORY","CONVERSATION_TYPE","COUNTRY","PHONE"])`);

            console.log('analytics.start(' + startdate + ').end(' + enddate + ').granularity(DAILY).phone_numbers(' + phonenumberArr + ').country_codes(' + country_code + ')')
            let fields = `conversation_analytics.start(${startdate}).end(${enddate}).granularity(DAILY).phone_numbers(${phonenumberArr}).dimensions(["CONVERSATION_CATEGORY","CONVERSATION_TYPE","COUNTRY","PHONE"])`
            // console.log(fields);
            // if (product_type == "0" || product_type == "2") {
            //     fields += '.product_types(' + product_type + ')';
            // }
            whatsappService.getMetricsanalytics(fields, access_token, wabainfo.wabaid, (err, result) => {
                if (err) {
                    // console.log(err)
                    ...
                    if (result.conversation_analytics != undefined && result.hasOwnProperty("conversation_analytics")) {
                        if (result.conversation_analytics.data[0].data_points != undefined) {
                            console.log("getMetrics=========== conversation_analytics inside else1", JSON.stringify(result))
                            let tmpData = result.conversation_analytics.data[0].data_points;

                            if (tmpData.length > 0) {

                                tmpData.forEach((item, index) => {
                                    console.log("item inside tempdata------------>", item)
